...
public final class SklepyRunner implements EndpointRunner, CreatesTestRecord {

    private static final Logger LOG = LoggerFactory.getLogger(SklepyRunner.class);
    private static final String ENDPOINT = "sklepy";
    private static final String IDS_KEY = "sklepy.id";
    private final DemoMode mode;
    private final SoftDeleteIds ids;
    private String createdId;

    public SklepyRunner(DemoMode mode, SoftDeleteIds ids) {
        this.mode = mode;
        this.ids = ids;
    }
    ...
    private static final String DEMO_NAZWA = "SDK Demo Sklep";
    private static final int DEMO_NUMER = 9999;
    private static final String DEMO_NAZWA_UPDATED = "SDK Demo Sklep Updated";
    ...
    private static final String LOG_UD_GET = "[{}] UD: getById({}) -> nazwa='{}', numer={}, aktywny={}";
    ...
    private static final String LOG_CREATE_ONCE = "[{}] CREATE-ONCE: create(nazwa='{}', numer={}) -> id={}";
    ...
    public void run(NoviCloudClient client) throws NoviCloudException {
        ...
        SklepyClient api = client.sklepy();
        runCount(api, read);
        runGetById(api, read);
        runListAll(api, read);
    ...
    private void runCount(SklepyClient api, boolean enabled) {
        if (!enabled) { return; }
        logCount(LOG, ENDPOINT, api.count(null));
    }

    private void runGetById(SklepyClient api, boolean enabled) {
        if (!enabled) { return; }
        PagedResult<Sklep> result = api.list(null);
        Iterator<Sklep> iterator = result.iterator();
        if (iterator.hasNext()) {
    ...
    private void runListAll(SklepyClient api, boolean enabled) {
        if (!enabled) { return; }
    ...
    private void runListPageAllFilters(SklepyClient api, boolean enabled) {
        if (!enabled) { return; }
        int count = api.count(SklepQueryBuilder.builder()
                .fts(FILTER_ALL_STR).id(FILTER_ALL_ID).nazwa(FILTER_ALL_STR)
                .nip(FILTER_ALL_STR).skrot(FILTER_ALL_STR).numer(FILTER_ALL_ID)
                .ulica(FILTER_ALL_STR).kodPoczt(FILTER_ALL_STR).miasto(FILTER_ALL_STR)
                .telefon(FILTER_ALL_STR).email(FILTER_ALL_STR).aktywny(false)
                .build());
        LOG.info(LOG_LIST_FILTERS, ENDPOINT, count);
    }

    private void runUpdateDelete(SklepyClient api, boolean enabled) {
        if (!enabled) {
        ...
        api.update(SklepUpdateBuilder.builder(id).nazwa(DEMO_NAZWA_UPDATED).build());
        ...
        api.update(SklepUpdateBuilder.builder(id).nazwa(fetched.nazwa()).aktywny(true).build());
    ...
    private void runCreateOnce(SklepyClient api, boolean enabled) {
        if (!enabled) { return; }
        this.createdId = api.create(SklepCreateBuilder.builder(DEMO_NAZWA, DEMO_NUMER).build());
        LOG.info(LOG_CREATE_ONCE, ENDPOINT, DEMO_NAZWA, DEMO_NUMER, createdId);
    }
