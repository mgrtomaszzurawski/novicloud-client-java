 ...
 * Implementation lives in the non-exported {@code sdk.internal.resources.kontrahenci}
 * package; external code should depend on this interface only.
 *
 * @since 2.0.0
 */
public interface KontrahenciClient {

    /**
     * Returns a lazy paginated result over all contractors matching the given filters.
     *
     ...
     */
    PagedResult<Kontrahent> list(KontrahentQueryBuilder query);

    /**
     * Returns the total number of contractors matching the given filters.
     *
     * @param query filter parameters, or {@code null} for no filtering
     * @return total record count
     * @throws NoviCloudException on API failure
     */
    int count(KontrahentQueryBuilder query);

    /**
     * Fetches a single contractors record by its numeric ID.
     *
     * @param id the record ID; must not be {@code null}
     * @return the record; never {@code null}
     * @throws IllegalArgumentException if {@code id} is {@code null}
     * @throws NoviCloudNotFoundException if no contractors record with the given ID exists
     * @throws NoviCloudException on other API failure
     */
    Kontrahent getById(Long id);

    /**
     * Creates a new contractors record.
     *
     ...
     * @throws NoviCloudException on API failure
     */
    String create(KontrahentCreateBuilder builder);

    /**
     * Updates an existing contractors record. The identifier field in the builder identifies the record.
     *
     * @param builder the updated data; must not be {@code null}
     * @throws NoviCloudNotFoundException if no contractors record with the given identifier exists
     * @throws NoviCloudException on other API failure
     */
    void update(KontrahentUpdateBuilder builder);
