 ...
 * @since 1.0.0
 */
public final class DokumentyClient {
    ...
    public DokumentyClient(ApiClient apiClient, String accountName, RetryPolicy retryPolicy) {
    ...
    public PagedResult<Dokument> list(DokumentQueryBuilder query) {
        ...
        return PagedResult.of(
                ...
                p -> {
                    var items = p.getDane();
                    return items == null ? List.of() : items.stream().map(Dokument::from).toList();
                },
                DokumentyClient::extractSelfLink,
