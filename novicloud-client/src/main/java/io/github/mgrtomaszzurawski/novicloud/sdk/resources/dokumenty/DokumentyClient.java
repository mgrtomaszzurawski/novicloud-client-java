 ...
 * Implementation lives in the non-exported {@code sdk.internal.resources.dokumenty}
 * package; external code should depend on this interface only.
 *
 * @since 2.0.0
 */
public interface DokumentyClient {

    /**
     * Returns a lazy paginated result over all documents matching the given filters.
     *
     ...
     */
    PagedResult<Dokument> list(DokumentQueryBuilder query);

    /**
     * Returns the total number of documents matching the given filters.
     *
     * @param query filter parameters, or {@code null} for no filtering
     * @return total record count
     * @throws NoviCloudException on API failure
     */
    int count(DokumentQueryBuilder query);

    /**
     * Fetches a single documents record by its numeric ID.
     *
     * @param id the record ID; must not be {@code null}
     * @return the record; never {@code null}
     * @throws IllegalArgumentException if {@code id} is {@code null}
     * @throws NoviCloudNotFoundException if no documents record with the given ID exists
     * @throws NoviCloudException on other API failure
     */
    Dokument getById(Long id);
}
