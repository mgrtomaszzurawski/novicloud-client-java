...
@WireMockTest
class AsortyClientIntegrationTest {

    private static final String URL_LIST = "/[^/]+/asorty";
    private static final String URL_BY_ID = "/[^/]+/asorty/[^/]+";
    private static final String LIST_FILE = "asorty/list.json";
    private static final String SINGLE_FILE = "asorty/single.json";

    private static final int EXPECTED_SIZE = 4;
    private static final long EXPECTED_FIRST_ID = 1L;

    // Expected field values
    private static final String EXPECTED_NAZWA_A = "Assortment A";
    private static final String EXPECTED_NAZWA_D = "Assortment D";
    private static final long EXPECTED_LAST_ID = 4L;
    private static final String CREATE_NAZWA = "New Assort";
    private static final String EXPECTED_CREATED_ID = "9999";
    private static final String UPDATE_NAZWA = "Updated";
    ...
    void list_whenServerReturnsData_deserializesAllFields() {
        ...
        // when
        List<Asorty> dane = new ArrayList<>();
        client.asorty().list(null).forEach(dane::add);

        // then
        assertEquals(EXPECTED_SIZE, dane.size());
        assertEquals(EXPECTED_FIRST_ID, dane.get(FIRST_INDEX).id());
        assertEquals(EXPECTED_NAZWA_A, dane.get(FIRST_INDEX).nazwa());
        assertEquals(EXPECTED_LAST_ID, dane.get(FOURTH_INDEX).id());
        assertEquals(EXPECTED_NAZWA_D, dane.get(FOURTH_INDEX).nazwa());
    }
    ...
    void count_whenServerReturnsData_returnsSizeField() {
        ...
        // when
        int result = client.asorty().count(null);
    ...
    void getById_whenRecordExists_deserializesAllFields() {
        ...
        // when
        Asorty a = client.asorty().getById(EXPECTED_FIRST_ID);

        // then
        assertNotNull(a);
        assertEquals(EXPECTED_FIRST_ID, a.id());
        assertEquals(EXPECTED_NAZWA_A, a.nazwa());
    }
    ...
    void create_whenServerAccepts_returnsCreatedId() {
        ...
        // when
        String id = client.asorty().create(
                AsortyCreateBuilder.builder(CREATE_NAZWA).build());
    ...
    void update_whenRecordExists_completesWithoutError() {
        ...
        // when / then
        assertDoesNotThrow(() -> client.asorty().update(
                AsortyUpdateBuilder.builder(EXPECTED_FIRST_ID).nazwa(UPDATE_NAZWA).build()));
    ...
    void deleteById_whenRecordExists_completesWithoutError() {
        ...
        // when / then
        assertDoesNotThrow(() -> client.asorty().deleteById(EXPECTED_FIRST_ID));
    ...
    void list_whenMultiplePages_iteratesAllPages(WireMockRuntimeInfo wm) {
        ...
        String page1 = """
                {
                  ...
                  "links": {
                    "self": "http://localhost:%d/demo/asorty?start=0",
                    "next": "http://localhost:%d/demo/asorty?start=2"
                  },
        ...
        String page2 = """
                {
                  ...
                  "links": {
                    "self": "http://localhost:%d/demo/asorty?start=2"
                  },
                  "dane": [{"id": 3, "nazwa": "C"}]
                }""".formatted(port);
        ...
        // when
        java.util.List<Asorty> all = new java.util.ArrayList<>();
        client.asorty().list(null).forEach(all::add);
    ...
    void list_whenFirstCallReturns500_retriesAndReturnsData(WireMockRuntimeInfo wm) {
        ...
        // when / then
        assertDoesNotThrow(() -> retryClient.asorty().count(null));
